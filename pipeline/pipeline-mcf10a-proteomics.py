...
def processDatasets(infiles, outfile):
	...
	# Read data
	cycif_dataframe = pd.read_excel(expression_infile, index_col='DrugName')[antibody_dict.keys()].rename(columns=antibody_dict)
...
def mergeGeneCoexpression(infiles, outfile):
	...
	for infile in infiles:
		...
		# Read dataframe
		correlation_dataframe = pd.read_table(infile)
